...
### One Final Chart: Productivity Over the Year

```{python}
#| fig-cap: "Daily productivity across all shops throughout the year"
#| fig-width: 5
#| fig-height: 11

fig, ax = plt.subplots(figsize=(5, 11))

# Plot each shop's productivity over time (observation = day)
colors_shops = ['#E74C3C', '#3498DB', '#2ECC71', '#9B59B6', '#F39C12']
shop_names = ['Shop 1', 'Shop 2', 'Shop 3', 'Shop 4', 'Shop 5']

for shop in range(1, 6):
    shop_data = carsDF[carsDF['shopID'] == shop].sort_values('observation')
    ax.plot(shop_data['observation'], shop_data['carsFixed'], 
            alpha=0.7, linewidth=1.5, color=colors_shops[shop-1], label=shop_names[shop-1])
    
    # Add markers for boss present days
    boss_present = shop_data[shop_data['boss'] == 1]
    ax.scatter(boss_present['observation'], boss_present['carsFixed'], 
               color=colors_shops[shop-1], s=50, edgecolors='black', linewidth=1, zorder=5)

ax.set_xlabel('Day of Year (Observation)', fontsize=12, fontweight='bold')
ax.set_ylabel('Cars Fixed', fontsize=12, fontweight='bold')
ax.set_title('Daily Productivity Across All Shops Over Time\n(Dots = Boss Present)', 
             fontsize=14, fontweight='bold')
