...
for i, shop in enumerate(range(1, 6)):
    ...
    for j, (boss_label, color) in enumerate(colors.items()):
        ...
        ax.scatter(x, data['carsFixed'], alpha=0.6, s=40, c=color, 
    ...
    # Draw mean lines
    ax.hlines(away_mean, -0.3, 0.3, colors='#E67E22', linewidth=2, linestyle='-')
    ax.hlines(present_mean, 0.7, 1.3, colors='#27AE60', linewidth=2, linestyle='-')
    ...
    ax.text(0.5, mid_y, f'Δ {delta_label}', ha='center', va='center', fontsize=9, 
    ...
    ax.set_xticks([0, 1])
    ax.set_xticklabels(['You Away', 'You Present'], fontsize=9)
    ax.set_ylabel('Cars Fixed', fontweight='bold', fontsize=9)
    ax.set_title(f'Shop {shop}', fontsize=12, fontweight='bold', color=shop_colors[i])
    ax.set_ylim(0, 75)
    ax.set_xlim(-0.5, 1.5)
    
    # Add sample size labels below x-axis labels
    ax.text(0, -6, f'(n={n_away})', ha='center', fontsize=8, style='italic')
    ax.text(1, -6, f'(n={n_present})', ha='center', fontsize=8, style='italic')

# Hide the 6th subplot and use it for legend
axes[5].axis('off')
from matplotlib.patches import Patch
legend_elements = [Patch(facecolor='#E67E22', label='You Away'),
                   Patch(facecolor='#27AE60', label='You Present')]
axes[5].legend(handles=legend_elements, loc='center', fontsize=11, title='Your Presence', 
               title_fontsize=12, frameon=True)

fig.suptitle('Daily Productivity Split by Shop: Boss Presence Delta', fontsize=14, fontweight='bold', y=0.98)
plt.tight_layout()
...
for idx, shop in enumerate(range(1, 6)):
    ...
    ax.text(0, -5, f'n={len(boss_away)}', ha='center', fontsize=8, style='italic')
    ax.text(1, -5, f'n={len(boss_present)}', ha='center', fontsize=8, style='italic')

# Hide the 6th subplot (empty)
axes[5].axis('off')

# Add a legend to the empty subplot area
from matplotlib.patches import Patch
...
axes[5].legend(handles=legend_elements, loc='center', fontsize=11, title='Legend', title_fontsize=12)
axes[5].text(0.5, 0.3, '◆ = Mean', ha='center', va='center', fontsize=10, transform=axes[5].transAxes)

fig.suptitle('Boss Presence Impact Across All Shops', fontsize=14, fontweight='bold', y=0.98)
plt.tight_layout()
plt.subplots_adjust(top=0.92)
plt.show()
```
